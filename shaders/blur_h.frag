...
const float tex_width = 1.0/640.0;
...
void main(void)
{
    ...
    {
        vec2 left_texcoord = vec2(vTexCoord.x - tex_width * i, vTexCoord.y);
        vec2 right_texcoord = vec2(vTexCoord.x + tex_width * i, vTexCoord.y);

        if (left_texcoord.x >= 0.0)
            blurColor += texture2D(mTex, left_texcoord) * gauss_nums[i];
        if (right_texcoord.x <= 1.0)
